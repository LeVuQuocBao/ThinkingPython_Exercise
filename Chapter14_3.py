'Borrow first part code from thinkpython.com for basic part'
...
def filter_length(d, n):
    ...
    for word, anagrams in d.items():
        if len(word) == n:
            res[word] = anagrams
    return res
def store_anagrams(dbname,d):
    ''' Take a input dict d and write it to dbname shelve'''
    #  Open  shelve
    ...
    for key,value in d.items():
        shelf[key]=value
    shelf.close
def read_anagrams(string,dbname):
    shelf=shelve.open(dbname)
    stringlist=list(string)
    stringlist.sort()
    stringjoin=''.join(stringlist)
    anagramout=shelf[stringjoin]
    return anagramout
d = all_anagrams('words.txt')
shelf=store_anagrams('data14_3exercise.db',d)
print(read_anagrams('opst','data14_3exercise.db'))
